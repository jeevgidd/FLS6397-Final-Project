...
The GPS trajectory file of each taxi contains four columns (refer user guide available in the taxi_data folder).
...
We extract the year, month, day, complete-time, only hours, only minutes, and only seconds information from the date_time column, and save each one of them in a new column. For this, we use the **hms** library functions. In addition, we save the day of the week information (Monday to Sunday) using the date from the date_time column.  
...
We now classify the GPS records into **stay points** and **moving points** (see definitions in introduction) using the algorithm proposed in a paper published by [Microsoft](https://www.microsoft.com/en-us/research/publication/mining-user-similarity-based-on-location-history/) [@li2008mining]. To assign a GPS point as a stay point, the algorithm uses two variables; the distance and the time difference.  

In our project, we define a GPS point as a stay point if the distance traveled from previous point is **less than or equal to 50 meters** and time taken to reach this point is **up to 30 minutes**. We use a threshold of 30 minutes of time difference, assuming that Beijing municipality observes heavy traffic (to avoid labeling moving points as stay points). The points other than the stay points are labeled as moving points.
...
As we can see, each GPS point is labeled either with 0 (stay point) or -1 (moving point). Our goal is to label multiple set of stay points as stay locations and multiple set of moving points as trips (see the definition in the introduction).
...
As seen in the summary statistics the trip distances (km) vary from `r trip_table %>% pull(trip_distance_kms) %>% min()` to `r trip_table %>% pull(trip_distance_kms) %>% max()` and the trip duration (hrs) vary from `r trip_table %>% pull(trip_duration_hrs) %>% min()` to `r trip_table %>% pull(trip_duration_hrs) %>% max()`. We generate a well formatted table using **kable** function representing the top ten trips based on the distance traveled. 
...
Now we assume that the stay location with maximum duration spent is the **home** of the taxi driver and the stay locations less than one kilometer of distance from the home location as locations near home. The rest of the stay locations are considered as waiting points, where the driver waited for certain duration to pick up the customers.
...
We combine these new stay type labels into the original data set.
```{r}
...
Before we plot the above GPS records on an interactive map, we import the shape files of China and filter only for Beijing municipality. Beijing municipality consists of multiple districts and Beijing district being the major economic corridor.
...
We here plot the Beijing geometry by district and all the GPS records of the taxi 48. Each district of Beijing is colored with a different color and GPS points vary color according to the type of point.
...
Zooming into the interactive map, we can see that the driver lives (red point: home location) in Shunyi district while seems to work more (grey points: moving points) in the Beijing district. Other stay locations (waiting points) are mostly in the Beijing district and on the way between Beijing and Shunyi districts. Surprisingly, we can also observe the waiting locations in the Beijing International Airport present in the outskirts of Shunyi district (bordering Beijing district).

<br>
**Aggregate Analysis**  
Now we will import 9 random taxi files and merge them into one single tibble.
```{r}
...
We combine the previously defined functions such as distance_speed, stay_moving, ordered_stay_move functions into one function called full_analysis. Further, we define the functions to generate the trip and stay tables.
```{r}
...
Now using the above defined functions, we perform the complete analysis on the imported taxis data to generate a complete trip table and a complete stay table consisting of trips and stay locations information from all the imported taxis.
...
First, let's look at the aggregate stay locations on a map. We use the shape files of China and the ggplot functions to plot a static map colored by type of the stay location.
...
We can see that 5 out of 9 home locations are present outside the Beijing district, whereas most of the waiting locations of the taxis are inside the Beijing district. At least based on trajectory data of these 9 taxis, the taxi drivers seem to work more in Beijing district.

To quantify the above results, we identify the number of waiting points per each district by joining the waiting point GPS records with the Beijing shape file.
...
Based on the above table, we can confirm that Beijing district observed the highest number of waiting points.

Now, we will work with the complete trip table. First, we plot the number of trips done on each day of the week using the day of the week information extracted during the generation of complete trip table.
...
Now, we plot the number of trips that started and ended on each hour of the day. We also divide the day into 5 parts (morning, afternoon, evening, night and late night), and plot the number trips that started and ended in each part of the day.
...
From the top plot, we can see that the number of trips that start and end gradually increase from 8 am and observe the highest pickups and drops between 1 pm and 9 pm. The lowest number of trips are seen during 1 am to 7 am. When categorized by part of the day, the highest number of trips are observed in the evening. 
...
From the above percentage distribution plot, we see that on all the days, at least 50% of the trips made by these nine taxis have a trip duration of more than 3 hours. Although there is no clear pattern, the number of more than 3 hour trips gradually decrease from Wednesday to Saturday.

Now, let's try to build a **Least Squares Regression** model based on the above complete trip table and predict the time duration of a given trip distance.
...
We seem to have a lot of outliers to build a Least Squares Regression model. Therefore, we log transform the values and plot the graph again to see if we can eliminate the outliers and use only certain values to build the model.
...
We clearly see points less than 3.5 on log trip distance scale and less than 1 on log trip duration scale seems to be responsible for the worst fit. We delete all these points and use the rest of the points to build a regression model.
...
Now, before performing the regression, we do a correlation test between the trip distance and trip duration.
...
As seen in the above result, there exists a positive correlation with an estimate of `r corr %>% pull(estimate)` between trip distance and trip duration. That is, when the trip distance increases, the trip duration also tend to increase.

Now, trip duration as our dependent variable and trip distance as our independent variable, we perform a Least Squares Regression analysis.
...
As seen in the above regression result, our model states that to travel 1 kilometer, a duration of 2.6 minutes are required by the taxi. Although, we should note that our residual error (36.153) seem to be higher. It was already obvious that our model will not perfect based on the above log transformed Trip distance vs trip duration graph. But we proceed to predict duration of a trip when the distance is provided to our regression model.
...
As seen in the above result, a trip of **5 km** would take around **26.4 minutes** (Speed: **`r round((5/26.4)*60, 2)` km/hour**) to complete. By a quick [Google search](https://qz.com/163178/a-big-reason-beijing-is-polluted-the-average-car-goes-7-5-miles-per-hour/), we can see that the average speed of a car is around **12 km/hour**, which seems to be close to our result.
