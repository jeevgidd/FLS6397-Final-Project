---
title: "Final project-FLS6397: Exploring Beijing Taxis GPS data"
...
```{r, echo = FALSE} 
# formats the final page with messages and warnings disabled
# echo TRUE indicates r chunks to appear on final page except the present chunk which is declared as FALSE
...
<br>
```{r}
# excludes any duplicate rows (coordinates recorded at same time).
# .keep_all = TRUE indicates the data_id column not to be excluded in final tibble

taxi_df1 <- taxi %>% 
  distinct(taxi_id, date_time, longitude,latitude, .keep_all = TRUE)
...
`r a-b` observations (rows) were identified as duplicated rows.
