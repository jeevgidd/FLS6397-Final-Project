...
#### Observations:  
The project "Exploring Beijing Taxis GPS data" is submitted as the final project for the course Análise de Dados para as Ciências Sociais (FLS6397) offered at the Department of Political Science, University of São Paulo. The project is developed based on the principles and techniques discussed in the course, prioritizing the clarity and organization of the script and the good practices of data science and programming. Therefore, certain parts of the analysis consider simple assumptions and may not represent the actual complexity of a scientific problem. The complete instructions are found [here](https://jonnyphillips.github.io/Ciencia_de_Dados/projeto.html).

do not forget to remove the set working directory
```{r}
setwd("D:/data/Disciplines/Data_Analysis_for_Social_Sciences/Challenges/Final-Project/")
```

### 1. Introduction
Global Positioning System (GPS) technology has revolutionized the field of navigation. 

### 2. Objectives
1. Calculate variables such as distance and speed from raw coordinate tracks of the taxis  
2. Identify staying points and moving points of the taxis  
3. Generate and categorize trips and staying locations of the taxis  
4. Predict trip duration using a linear regression model  
...
As per the official user_guide (available in the taxi_data folder), the raw GPS trajectory of a taxi contains four columns.  
  * Taxi ID  
  * Date and time when the coordinates were recorded  
  * Longitude  
  * Latitude  
First, we import and work only with data from one taxi (TAXI 48) and assign respective column names.
...
We check if any of the GPS records are duplicated. Duplicate rows imply the same coordinates recorded at the same time. We use distinct() function eliminate duplicated rows. 
...
`r a-b` observations (rows) were identified as duplicated rows. Therefore we use the new tibble generated after applying distinct() function for the rest of our analysis. We also add a new column consisting of row numbers which serves as unique ID numbers for our GPS records.
...
We extract time, year, month, day, hours, minutes and seconds information from date_time column and place each one of them in a new column. For this, we use hms library functions. In addition, we also save the day of the week information (Monday to Sunday) using the date information from date_time column.  
<br>
```{r}
taxi_df1 <- taxi_df1 %>%
...
We create a geometry column using the longitude and latitude columns in the tibble. 
<br>
```{r}
